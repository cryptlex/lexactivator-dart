// AUTO GENERATED FILE, DO NOT EDIT.
//
// Generated by `package:ffigen`.
// ignore_for_file: constant_identifier_names
part of '../lexactivator.dart';

abstract class LexStatusCodes {
  /// Success code.

  static const int LA_OK = 0;

  /// Failure code.

  static const int LA_FAIL = 1;

  /// The license has expired or system time has been tampered with. Ensure your
  /// date and time settings are correct.

  static const int LA_EXPIRED = 20;

  /// The license has been suspended.

  static const int LA_SUSPENDED = 21;

  /// The grace period for server sync is over.

  static const int LA_GRACE_PERIOD_OVER = 22;

  /// The trial has expired or system time has been tampered with. Ensure your date
  /// and time settings are correct.

  static const int LA_TRIAL_EXPIRED = 25;

  /// The local trial has expired or system time has been tampered with. Ensure
  /// your date and time settings are correct.

  static const int LA_LOCAL_TRIAL_EXPIRED = 26;

  /// A new update is available for the product. This means a new release
  /// has been published for the product.

  static const int LA_RELEASE_UPDATE_AVAILABLE = 30;

  /// No new update is available for the product. The current version is
  /// @deprecated
  /// latest.

  static const int LA_RELEASE_NO_UPDATE_AVAILABLE = 31;

  /// The update available is not allowed for this license.

  static const int LA_RELEASE_UPDATE_NOT_AVAILABLE = 31;

  /// The update available is not allowed for this license.

  static const int LA_RELEASE_UPDATE_AVAILABLE_NOT_ALLOWED = 32;

  /// Invalid file path.

  static const int LA_E_FILE_PATH = 40;

  /// Invalid or corrupted product file.

  static const int LA_E_PRODUCT_FILE = 41;

  /// Invalid product data.

  static const int LA_E_PRODUCT_DATA = 42;

  /// The product id is incorrect.

  static const int LA_E_PRODUCT_ID = 43;

  /// Insufficient system permissions. Occurs when LA_SYSTEM flag is used
  /// but application is not run with admin privileges.

  static const int LA_E_SYSTEM_PERMISSION = 44;

  /// No permission to write to file.

  static const int LA_E_FILE_PERMISSION = 45;

  /// Fingerprint couldn't be generated because Windows Management
  /// Instrumentation (WMI) service has been disabled. This error is specific to
  /// Windows only.

  static const int LA_E_WMIC = 46;

  /// The difference between the network time and the system time is more
  /// than allowed clock offset.

  static const int LA_E_TIME = 47;

  /// Failed to connect to the server due to network error.

  static const int LA_E_INET = 48;

  /// Invalid network proxy.

  static const int LA_E_NET_PROXY = 49;

  /// Invalid Cryptlex host url.

  static const int LA_E_HOST_URL = 50;

  /// The buffer size was smaller than required.

  static const int LA_E_BUFFER_SIZE = 51;

  /// App version length is more than 256 characters.

  static const int LA_E_APP_VERSION_LENGTH = 52;

  /// The license has been revoked.

  static const int LA_E_REVOKED = 53;

  /// Invalid license key.

  static const int LA_E_LICENSE_KEY = 54;

  /// Invalid license type. Make sure floating license is not being used.

  static const int LA_E_LICENSE_TYPE = 55;

  /// Invalid offline activation response file.

  static const int LA_E_OFFLINE_RESPONSE_FILE = 56;

  /// The offline activation response has expired.

  static const int LA_E_OFFLINE_RESPONSE_FILE_EXPIRED = 57;

  /// The license has reached it's allowed activations limit.

  static const int LA_E_ACTIVATION_LIMIT = 58;

  /// The license activation was deleted on the server.

  static const int LA_E_ACTIVATION_NOT_FOUND = 59;

  /// The license has reached it's allowed deactivations limit.

  static const int LA_E_DEACTIVATION_LIMIT = 60;

  /// Trial not allowed for the product.

  static const int LA_E_TRIAL_NOT_ALLOWED = 61;

  /// Your account has reached it's trial activations limit.

  static const int LA_E_TRIAL_ACTIVATION_LIMIT = 62;

  /// Machine fingerprint has changed since activation.

  static const int LA_E_MACHINE_FINGERPRINT = 63;

  /// Metadata key length is more than 256 characters.

  static const int LA_E_METADATA_KEY_LENGTH = 64;

  /// Metadata value length is more than 256 characters.

  static const int LA_E_METADATA_VALUE_LENGTH = 65;

  /// The license has reached it's metadata fields limit.

  static const int LA_E_ACTIVATION_METADATA_LIMIT = 66;

  /// The trial has reached it's metadata fields limit.

  static const int LA_E_TRIAL_ACTIVATION_METADATA_LIMIT = 67;

  /// The metadata key does not exist.

  static const int LA_E_METADATA_KEY_NOT_FOUND = 68;

  /// The system time has been tampered (backdated).

  static const int LA_E_TIME_MODIFIED = 69;

  /// Invalid version format.

  static const int LA_E_RELEASE_VERSION_FORMAT = 70;

  /// Incorrect email or password.

  static const int LA_E_AUTHENTICATION_FAILED = 71;

  /// The meter attribute does not exist.

  static const int LA_E_METER_ATTRIBUTE_NOT_FOUND = 72;

  /// The meter attribute has reached it's usage limit.

  static const int LA_E_METER_ATTRIBUTE_USES_LIMIT_REACHED = 73;

  /// Custom device fingerprint length is less than 64 characters
  /// or more than 256 characters.

  static const int LA_E_CUSTOM_FINGERPRINT_LENGTH = 74;

  /// No product version is linked with the license.

  static const int LA_E_PRODUCT_VERSION_NOT_LINKED = 75;

  /// The product version feature flag does not exist.

  static const int LA_E_FEATURE_FLAG_NOT_FOUND = 76;

  /// Application is being run inside a virtual machine / hypervisor, and
  /// activation has been disallowed in the VM.

  static const int LA_E_VM = 80;

  /// Country is not allowed.

  static const int LA_E_COUNTRY = 81;

  /// IP address is not allowed.

  static const int LA_E_IP = 82;

  /// Application is being run inside a container and
  /// activation has been disallowed in the container.

  static const int LA_E_CONTAINER = 83;

  /// Rate limit for API has reached; try again later.

  static const int LA_E_RATE_LIMIT = 90;

  /// Server error.

  static const int LA_E_SERVER = 91;

  /// Client error.

  static const int LA_E_CLIENT = 92;

  /// Invalid account ID.

  static const int LA_E_ACCOUNT_ID = 93;

  /// The user account has been temporarily locked for 5 mins due to 5 failed attempts.

  static const int LA_E_LOGIN_TEMPORARILY_LOCKED = 100;

  /// Invalid authentication ID token.

  static const int LA_E_AUTHENTICATION_ID_TOKEN_INVALID = 101;

  /// OIDC SSO is not enabled.

  static const int LA_E_OIDC_SSO_NOT_ENABLED = 102;

  /// The allowed users for this account has reached its limit.

  static const int LA_E_USERS_LIMIT_REACHED = 103;

  /// OS user has changed since activation and the license is user-locked.

  static const int LA_E_OS_USER = 104;

  /// Invalid permission flag.

  static const int LA_E_INVALID_PERMISSION_FLAG = 105;

  /// The free plan has reached it's activation limit.

  static const int LA_E_FREE_PLAN_ACTIVATION_LIMIT_REACHED = 106;

  /// Invalid feature entitlements.

  static const int LA_E_FEATURE_ENTITLEMENTS_INVALID = 107;

  /// The feature entitlement does not exist.

  static const int LA_E_FEATURE_ENTITLEMENT_NOT_FOUND = 108;

  /// No entitlement set is linked to the license.
  
  static const int LA_E_ENTITLEMENT_SET_NOT_LINKED = 109;
}
